...
# Load data
D <- readRDS("/Users/Madshove/Dropbox/Statskundskab/4V/data/Ads_2021-04-20.rds")
B <- readRDS("/Users/Madshove/Dropbox/Statskundskab/4V/data/Ads_2021-04-20.rds") # used for descriptive analysis purpose

#### FILTER ONLY ADS BEFORE ELECTION BUT AFTER 1st APRIL ####
D <- D %>%
  filter(ad_creation_time <= "2019-06-04") # We chose to say that election day did count as after election

D <- D %>%
  filter(ad_creation_time >= "2019-04-01")
...
# Remove duplicates so we can join in other variables easy
D <- D[!duplicated(D$page_id), ]

# Load dataframe from other project
D0 <- readRDS("/Users/Madshove/Dropbox/Statskundskab/4V/data/FB_VOTE_final.rds") # This one is used for a previously coded name variable that is good to have
D0 <- D0[!duplicated(D0$page_id), ]

# Add variables from old dataframe
D <- merge(x = D, y = D0 [ , c("page_id", "Navn", "Parti_19", "Storkreds_2019", "year_in_parliament", "PV_2015", "PV_2019", "kredsmandatfv19", "tillaegsmandatfv19", "date")], by = "page_id")
